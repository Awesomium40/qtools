...
class SurveyObject(OrderedDict):
    ...
    def __setattr__(self, key, value):
        ...
        if key in self.keys():
            raise AttributeError("Setting of attribute {0} not supported".format(key))

        setattr(super(SurveyObject, self), key, value)
    ...
    def _sanitize_for_spss_(dirty_str, sub=None):
        ...
        if len(new_var) == 0:
            SurveyObject.__counter__ += 1
            new_var = "VAR_{0}".format(SurveyObject.__counter__)
        ...
        if len(subs) > 0:
            new_var = SurveyObject._multi_replace_(new_var, subs)

        # locate invalid characters and replace with underscores
        replacements = {x: "_" for x in invalid_chars.findall(new_var)}
        new_var = SurveyObject._multi_replace_(new_var, replacements) if len(replacements) > 0 else new_var

        return new_var


class SurveyQuestion(SurveyObject):
    ...
    def validation(self):
        settings: SurveyObject = self.Payload.Validation.Settings
